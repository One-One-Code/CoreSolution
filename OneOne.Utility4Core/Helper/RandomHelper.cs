 ...
 * ------------------修改记录-------------------
 * 修改人      修改日期        修改目的
 * 丁静        2017-04-14     修改GetCode方法，生成的随机字符串长度根据按照传入的长度为准
 ----------------------------------------------------*/
...
namespace OneOne.Utility4Core.Helper
{
    ...
    public class RandomHelper
    {
        ...
        {
            var chars = InitCodeChar();
            return GetCode(chars, codeLength);
        }
        /// <summary>
        /// 初始化激活码组成元素的数组，从0-9和A-Z
        /// </summary>
        private static List<char> InitCodeChar()
        {
            var codeChar = new List<char>();
            if (codeChar.Count != 32)
            {
                codeChar.Clear();
            }
            for (char i = '2'; i <= '9'; i++)
        ...
        /// </summary>
        /// <returns></returns>
        private static string GetCode(List<char> codeChar, int codeLength)
        {
            var sb = new StringBuilder(codeLength);
            var rd = new Random();
            for (int i = 0; i < codeLength; i++)
            {
                int index = rd.Next(0, 31);
                var value = codeChar[index];
                sb.Append(value);
            }
            return sb.ToString();
        }
